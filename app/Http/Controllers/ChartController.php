...
class ChartController extends Controller
{
    ...
    public function statistic2()
    {
        ...
        $that_nghiep = DB::table('students')->where('job', '=', 'Thất nghiệp')->count('id');
        $dang_lam_viec = DB::table('students')->where('job', '<>', 'Thất nghiệp')->count('id');

        // thong ke sinh vien theo tinh thanh pho 
        $hanam = DB::table('students')->where('address', '=', 'Hà Nam')->count('id');
        $hanoi = DB::table('students')->where('address', '=', 'Hà Nội')->count('id');
